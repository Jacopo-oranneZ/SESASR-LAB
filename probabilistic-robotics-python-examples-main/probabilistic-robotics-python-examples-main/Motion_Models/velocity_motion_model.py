...
def sample_velocity_motion_model(x, u, a, dt):
    ...
    """
    v_hat = u[0] + np.random.normal(0, a[0]*u[0]**2 + a[1]*u[1]**2)  # linear velocity with noise(normal distribution centered in 0)
    w_hat = u[1] + np.random.normal(0, a[2]*u[0]**2 + a[3]*u[1]**2)  # angular velocity with noise(normal distribution centered in 0)
    gamma_hat = np.random.normal(0, a[4]*u[0]**2 + a[5]*u[1]**2)

    # Formuale due to geometical problem behavior of robot motion
    r = v_hat/w_hat
...
def main():
    ...
    x = [2, 4, 0] # initial pose of the robot
    u = [0.8, 0.6] # velocity commands [v, w]
    a = [0.001, 0.01, 0.1, 0.2, 0.05, 0.05] # noise variance (apriori information)
...
# This is what would happen if there is no filtering
