...
# Measurement sigma (range, bearing) -> Sensor Noise
SIGMA_MEASUREMENT = [0.1, 0.05] # [meters, radians]

# Definition of the EKF Node
class EKFnode(Node):
    def __init__(self):
        ...
        self.sigma_u = np.array([0.0, 0.0]) # Current noise - noise parameters of the motion model [a1, a2, a3, a4, a5, a6] expressed as [std_dev_v, std_dev_w]

        # Definition of the time step - we want to perform the prediction step at a fixed rate of 20 Hz
        self.dt = 0.05 # 20 Hz

        # 3. PUB AND SUBSCRIBE
        self.ekf_pub = self.create_publisher(Odometry, '/ekf', 10) # Publisher for the EKF state
    ...
    # ---------------------------------------------------------
    # TIMER PREDICTION (Performs the motion step every 0.05s)
    # ---------------------------------------------------------
    def prediction_callback(self):
        # Execute prediction step
        # Pass dt as a tuple in g_extra_args
    ...
    # ---------------------------------------------------------
    # LANDMARK CALLBACK (Performs the update if something is seen)
    # ---------------------------------------------------------
    def landmark_callback(self, msg):
        # Measurement noise covariance matrix Q (fixed)
        Q = np.diag([SIGMA_MEASUREMENT[0]**2, SIGMA_MEASUREMENT[1]**2])  # Diag(sigma_range^2, sigma_bearing^2)
