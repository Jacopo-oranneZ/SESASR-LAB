...
class DWA():
    ...
    def go_to_pose(self, goal_pose):
        ...
        if goal_pose is list:
            goal_pose = np.array(goal_pose) # ensure numpy array

        success = False
        dist_to_goal = np.linalg.norm(self.robot.pose[0:2] - goal_pose) # initial distance

        print("Initial distance to goal: ", dist_to_goal)
        print("Initial Robot pose: ", self.robot.pose)

        steps = 1
        # When implementing in ROS, this loop has to be a timer callback
